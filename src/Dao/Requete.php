...
class Requete {
    public const SEL_TYPE_BASE = "SELECT id, nom, version 
                                    FROM type";
    public const SEL_CRON = "SELECT id, nom, recurrence, date_demarrafe 
                            FROM tache_cron";
    ...
    public const INS_DATABASE = "INSERT INTO client_database (nom,password, port,url, used_type, user_database
    values(?,?,?,?,?,?)";
}
